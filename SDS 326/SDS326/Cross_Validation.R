...
pOrig <- ggplot(dataOrig, aes(x = x, y = y)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  geom_line(data = popLine, aes(x = x, y = y), color = "red") +
  labs(title = "Original Noise", x = "x", y = "y")
print(pOrig)
print(summary(lmFit))

# (g)
lmPoly <- lm(y ~ x + I(x^2))
print(summary(lmPoly))

# (h) 
set.seed(1)
epsLess <- rnorm(n, 0, sqrt(0.05))
yLess <- -1 + 0.5*x + epsLess
dataLess <- data.frame(x = x, y = yLess)
lmFitLess <- lm(y ~ x, data = dataLess)
popLineLess <- data.frame(x = seq(min(x), max(x), length.out = 100))
popLineLess$y <- -1 + 0.5 * popLineLess$x

pLess <- ggplot(dataLess, aes(x = x, y = y)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  geom_line(data = popLineLess, aes(x = x, y = y), color = "red") +
  labs(title = "Less Noise", x = "x", y = "y")
print(pLess)
print(summary(lmFitLess))

# (i) 
set.seed(1)
epsMore <- rnorm(n, 0, 1)
yMore <- -1 + 0.5*x + epsMore
dataMore <- data.frame(x = x, y = yMore)
lmFitMore <- lm(y ~ x, data = dataMore)
popLineMore <- data.frame(x = seq(min(x), max(x), length.out = 100))
popLineMore$y <- -1 + 0.5 * popLineMore$x

pMore <- ggplot(dataMore, aes(x = x, y = y)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  geom_line(data = popLineMore, aes(x = x, y = y), color = "red") +
  labs(title = "More Noise", x = "x", y = "y")
print(pMore)
print(summary(lmFitMore))

# (j)
ciOrig <- confint(lmFit)
ciLess <- confint(lmFitLess)
ciMore <- confint(lmFitMore)
ciOrig
ciLess
ciMore
