...
# GAM AIC
aicGAM <- AIC(gamModel)
cat("GAM: AIC =", round(aicGAM, 2), "\n\n")
```

In order to explore potential nonlinearity in alcohol’s effect on white‐wine quality, we fitted a generalized additive model (GAM) with a spline on alcohol and linear terms for volatile acidity, sulfates, and residual sugar using 4,898 observations.
All linear predictors were highly significant, volatile acidity (beta = −2.17, p \< 2 × 10⁻¹⁶), sulfates (beta = 0.51, p = 7.45 × 10⁻⁸), and residual sugar (beta = 0.021, p \< 2 × 10⁻¹⁶).
The smooth term for alcohol exhibited an estimated 7.48 degrees of freedom (F = 180.2, p \< 2 × 10⁻¹⁶), revealing a clear diminishing returns pattern, which shows rapid quality gains at low alcohol levels (8–9% Alcohol by Volume, ABV), gradually smaller improvements through the mid range (9–11% ABV), and an almost flat response beyond ≈12% ABV.
Compared to the baseline OLS model (Adjusted R² = 0.2652, AIC = 11 186.5, Residual Std. Error = 0.7574), the GAM improved model fit (Adjusted R² = 0.2708, AIC = 11 178.2, Residual Std. Error = 0.7564) without undue complexity.
These findings indicate that targeting moderate alcohol levels ( 9–11% ABV) yields the greatest perceived quality benefit, whereas further increases provide minimal sensory advantages and may even risk off flavors.
Capturing this nonlinearity is therefore essential for optimizing fermentation strategies, and future work should validate these results via 10‐fold cross validation, examine potential interactions, and test on wines from different vintages or regions to ensure generalizability.
...
In our secondary analysis, we fitted a quadratic regression of wine quality on alcohol (both linear and squared) along with volatile acidity, sulfates, and residual sugar.
The squared‐alcohol term was highly significant (beta2 = 0.0425, p \< 10⁻⁸), while the linear alcohol term itself was negative (beta1 = –0.5503, p = 6×10⁻⁴), together producing a classic concave up curve; the quality rises sharply with initial increases in alcohol, then the marginal benefit tapers off at higher ABV levels.
Compared to the baseline OLS, the polynomial model yielded a slight but meaningful improvement in fit metrics, adjusted R² increased from 0.2652 to 0.2662, RMSE fell from 0.7574 to 0.7582, and AIC dropped in kind.
Crucially, a 10 fold cross validation confirmed that this quadratic specification achieved the lowest average RMSE of the three approaches, OLS, polynomial, and GAM, demonstrating that incorporating a simple curvature term can enhance out of sample predictive accuracy without overfitting.
