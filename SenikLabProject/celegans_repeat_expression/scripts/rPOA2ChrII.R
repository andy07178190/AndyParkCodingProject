...
plot_data_chrII <- merged_repeat_elements_and_counts %>%
  filter(Chr == "chrII") %>%                  # Keep chrII
...
max_val_chrII <- max(abs(plot_data_chrII$rpoa2_raw), abs(plot_data_chrII$N2_raw))

# Set the threshold for labeling (e.g., 3000)
threshold <- 3000

# Identify repeats exceeding the threshold in both groups
label_data_chrII <- plot_data_chrII %>%
...
bidirectional_plot_chrII <- ggplot(plot_data_chrII) +
  ...
  scale_x_continuous(
    breaks = seq(chrII_start, chrII_stop, by = 1e6),
    labels = unit_format(unit = "M", scale = 1e-6)
  ) +
  # y-axis: from -max_val_chrII to max_val_chrII, label as absolute values
  scale_y_continuous(
    name = "Raw counts",
    labels = function(x) abs(x),
    limits = c(-max_val_chrII, max_val_chrII),
    expand = expansion(mult = c(0.02, 0.02))
  ) +
  # Reference line at y=0 and rpoa2 start
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_vline(xintercept = rpoa2_start, color = "red", linetype = "dashed") +
  coord_cartesian(xlim = c(chrII_start, chrII_stop)) +
  theme_classic() +
...
# Display the plot
print(bidirectional_plot_chrII)

# Extract elements on chrII that exceed 3000 in both groups, then write to BED
common_elements_chrII <- plot_data_chrII %>%
  filter(rpoa2_avg > 3000, N2_avg > 3000)

common_elements_chrII_bed <- common_elements_chrII %>%
  select(Chr, Start, End, RepeatID, Strand)

write.table(
  common_elements_chrII_bed,
  file = file.path("/Users/baghuijae/Desktop/Github/celegans_repeat_expression/results", "common_elements_chrII_gt3000.bed"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = FALSE
